...
if ($resultEvaluasi->num_rows > 0) {
    ...
    $queryUpdate = "UPDATE tb_evaluasi 
                    SET evaluasi_tugas = ?, evaluasi_kehadiran = ?, evaluasi_total = ?
                    WHERE id_kelas_assesment = ? AND id_murid = ?";
    $stmtUpdate = $conn->prepare($queryUpdate);
    $stmtUpdate->bind_param("dddis", $evaluasi_tugas, $evaluasi_kehadiran, $evaluasi_total, $id_kelas, $id_murid);
    $stmtUpdate->execute();
} else {
    // Data evaluasi belum ada, lakukan insert
    $queryInsert = "INSERT INTO tb_evaluasi (id_kelas_assesment, id_murid, evaluasi_tugas, evaluasi_kehadiran, evaluasi_total) 
                    VALUES (?, ?, ?, ?, ?)";
    $stmtInsert = $conn->prepare($queryInsert);
    $stmtInsert->bind_param("iiidd", $id_kelas, $id_murid, $evaluasi_tugas, $evaluasi_kehadiran, $evaluasi_total);
    $stmtInsert->execute();
}
