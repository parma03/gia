    ...
    if ($data['count'] > 0) {
        ...
        if (move_uploaded_file($file_temp, $file_path)) {
            ...
            if ($stmt->execute()) {
                ...
                $query_admin = "INSERT INTO tb_admin (id_user, nama, profile) VALUES (?, ?, ?)";
            ...
            if ($stmt->execute()) {
                $last_id = $stmt->insert_id;
                $query_admin = "INSERT INTO tb_admin (id_user, nama) VALUES (?, ?)";
                $stmt = $conn->prepare($query_admin);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit'])) {
    ...
    $query_get_old_file = "SELECT profile FROM tb_admin WHERE id_user = ?";
    ...
    if ($data['count'] > 1) {
        ...
        if (!empty($file_name) && move_uploaded_file($file_temp, $file_path)) {
            ...
            if ($stmt->execute()) {
                $query_admin = "UPDATE tb_admin SET nama = ?, profile = ? WHERE id_user = ?";
                $stmt = $conn->prepare($query_admin);
            ...
            $stmt->bind_param("ssi", $username, $password, $id_user);
            if ($stmt->execute()) {
                $query_admin = "UPDATE tb_admin SET nama = ? WHERE id_user = ?";
                $stmt = $conn->prepare($query_admin);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    ...
    $query_select = "SELECT profile FROM tb_admin WHERE id_user = ?";
...
<head>
    ...
    <title>Data Admin - <?php echo $role; ?> | Global Intelligence Academy</title>
