    ...
    if ($data['count'] > 0) {
        ...
        if (move_uploaded_file($file_temp, $file_path)) {
            ...
            if ($stmt->execute()) {
                ...
                $query_admin = "INSERT INTO tb_guru (id_user, nama, profile) VALUES (?, ?, ?)";
                $stmt = $conn->prepare($query_admin);
                $stmt->bind_param("iss", $last_id, $nama, $random_name);
                if ($stmt->execute()) {
                    $_SESSION['notification'] = "Data Guru berhasil ditambah.";
            ...
            if ($stmt->execute()) {
                $last_id = $stmt->insert_id;
                $query_admin = "INSERT INTO tb_guru (id_user, nama) VALUES (?, ?)";
                $stmt = $conn->prepare($query_admin);
    ...
    if ($data['count'] > 1) {
        ...
        if (!empty($file_name) && move_uploaded_file($file_temp, $file_path)) {
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->execute()) {
                    $_SESSION['notification'] = "Data Guru berhasil diupdate.";
                    $_SESSION['alert'] = "alert-success";
                    redirect('admin/user/guru.php');
                    exit();
                } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    ...
    $query_select = "SELECT profile FROM tb_guru WHERE id_user = ?";
...
$query = "SELECT * FROM tb_user INNER JOIN tb_guru ON tb_user.id_user = tb_guru.id_user";
...
<head>
    ...
    <title>Data Guru - <?php echo $role; ?> | Global Intelligence Academy</title>
                                                                    ...
                                                                    <div class="form-group">
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            <input type="text" class="form-control"
                                                                                id="floatingInput"
                                                                                placeholder="Masukan Nama Guru" name="nama"
